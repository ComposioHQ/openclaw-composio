...
OpenClaw is a personal AI assistant you run on your own devices. It connects to WhatsApp, Telegram, Slack, Discord, and dozens of other channels. This fork adds [Composio](https://composio.dev) integration to make setup faster and credentials more secure.

## Why Composio?

Setting up OpenClaw normally requires configuring dozens of API keys and credentials for each integration. Composio simplifies this:

- **Faster setup**: Authenticate tools at runtime through Composio's unified auth flow — no more copy-pasting dozens of API keys during initial configuration
- **Secure credential handling**: API keys and OAuth tokens are managed securely by Composio's infrastructure, never stored in plaintext config files
- **Simplified maintenance**: Update credentials in one place, automatically propagates to all your integrations

## Quick Start

```bash
# Clone and install
git clone https://github.com/ComposioHQ/openclaw-composio.git
cd openclaw-composio
...
1. **Model API key** (Anthropic or OpenAI)
2. **Composio API key** (get from [platform.composio.dev/settings](https://platform.composio.dev/settings))

That's it. When your agent needs a tool (Gmail, GitHub, Slack, etc.), Tool Router automatically:
1. Finds the right tool for the task
2. Prompts you with an OAuth link if authentication is needed
3. Executes the action after you authenticate

Composio is **enabled by default** in this fork. To disable it:
```bash
pnpm moltbot config set plugins.composio.enabled false
```

For full plugin docs, see [extensions/composio/README.md](./extensions/composio/README.md).
...
<p align="center">
  <img src="https://raw.githubusercontent.com/clawdbot/clawdbot/main/docs/whatsapp-clawd.jpg" alt="OpenClaw" width="400">
</p>
