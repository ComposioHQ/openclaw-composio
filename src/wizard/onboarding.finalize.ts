...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  // Composio Tool Router - key was prompted early in onboarding
  const composioEntry = nextConfig.plugins?.entries?.composio as
    | { enabled?: boolean; config?: { apiKey?: string } }
    | undefined;
  const composioApiKey = (composioEntry?.config?.apiKey ?? "").trim();
  const composioEnv = (process.env.COMPOSIO_API_KEY ?? "").trim();
  const hasComposioKey = Boolean(composioApiKey || composioEnv);

  if (hasComposioKey) {
    await prompter.note(
      [
        "Composio Tool Router is ready.",
        "When your agent needs a tool (Gmail, GitHub, Slack, etc.),",
        "it will automatically prompt you to OAuth if needed.",
        "",
