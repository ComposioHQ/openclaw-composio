...
export async function runOnboardingWizard(
  ...
  // Composio API Key - early in onboarding for this fork
  type ComposioPluginEntry = { enabled?: boolean; config?: { apiKey?: string } };
  const existingComposioEntry = nextConfig.plugins?.entries?.composio as
    | ComposioPluginEntry
    | undefined;
  const existingComposioKey =
    existingComposioEntry?.config?.apiKey ?? process.env.COMPOSIO_API_KEY ?? "";
  const composioKeyInput = await prompter.text({
    message: "Composio API key (platform.composio.dev/settings) - Enter to skip",
    initialValue: existingComposioKey,
  });
  if (composioKeyInput.trim()) {
    const composioEntry = {
      enabled: true,
      config: {
    ...
    const existingEntries = nextConfig.plugins?.entries ?? {};
    nextConfig = {
      ...nextConfig,
      plugins: {
        ...nextConfig.plugins,
  ...
  if (opts.skipChannels ?? opts.skipProviders) {
    ...
    // Composio fork: default Telegram to open access (no pairing required)
    if (flow === "quickstart" && nextConfig.channels?.telegram?.enabled) {
      nextConfig = {
        ...nextConfig,
        channels: {
          ...nextConfig.channels,
